...
class HwkjpayWxpay implements IComplain
{
    ...
    public function getNewInfo($id){
        ...
        try{
            ...
            $info = $result['data'][0];
            $replys = $this->service->execute('trade.complaint.history', $params);
        } catch (Exception $e) {
            return ['code'=>-1, 'msg'=>$e->getMessage()];
        }
        ...
        if(!empty($replys['data'])){
            foreach($replys['data'] as $row){
                ...
                if($row['operator']=='投诉人' && $i == 1){
                    ...
                    $data['reply_detail_infos'][] = ['type'=>self::getUserType($row['operator']), 'name'=>$row['operator'], 'time'=>$time, 'content'=>$row['operateDetails'], 'images'=>[]];
                }
            }
        }
    ...
    public function complete($thirdid){
        ...
        try{
            $result = $this->service->execute('trade.complaint.close', $params);
            return ['code'=>0, 'data'=>$result];
        } catch (Exception $e) {
            return ['code'=>-1, 'msg'=>$e->getMessage()];
        }
